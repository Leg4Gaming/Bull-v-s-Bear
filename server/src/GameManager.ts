...
export class GameManager {
    ...
    public tryStartWaitingLobby() {
        if (this.gameState.phase === 'PRE_MATCH' && this.gameState.subPhase === 'WAITING_LOBBY' && !this.preMatchTimer) {
            console.log('Starting Waiting Lobby...');
            this.runSubPhase('WAITING_LOBBY', 10, () => {
                this.startPreMatch();
            });
        }
    }
        ...
        this.runSubPhase('INTRO', 3, () => {
            this.runSubPhase('AVATAR_SELECTION', 15, () => {
                this.runSubPhase('STRATEGY_SELECTION', 15, () => {
                    // Deterministic scenario selection based on game ID
                    let hash = 0;
                    for (let i = 0; i < this.gameState.id.length; i++) hash = (hash << 5) - hash + this.gameState.id.charCodeAt(i);
                    const prng = Math.abs(Math.sin(hash));
                    this.gameState.activeScenario = SCENARIOS[Math.floor(prng * SCENARIOS.length)];
    ...
    private startRound() {
        ...
        this.currentNewsCard = getRandomNewsCard(this.gameState.id, this.gameState.currentRound);
    ...
    private updatePricesGameMode(tradingSecondsElapsed: number) {
        ...
        this.gameState.assets.forEach(asset => {
            ...
            // 4. RANDOM NOISE (ε): Market unpredictability (Gaussian-like) -> Changed to deterministic sine wave
            const epsilon = this.generateDeterministicNoise(asset.type, asset.id, tradingSecondsElapsed);
    ...
    // Generate deterministic noise (ε) using sine waves
    private generateDeterministicNoise(assetType: string, assetId: string, tradingSecondsElapsed: number): number {
        const range = RANDOM_NOISE_RANGE[assetType] || 0.005;
        
        let hash = 0;
        for (let i = 0; i < assetId.length; i++) hash = (hash << 5) - hash + assetId.charCodeAt(i);
        const phaseShift = Math.abs(hash % 100);

        // Sum of two sine waves for a realistic but predictable wiggle
        const wave1 = Math.sin((tradingSecondsElapsed + phaseShift) * 0.5);
        const wave2 = Math.sin((tradingSecondsElapsed * 1.2 + phaseShift) * 0.2);
        
        const combinedWave = (wave1 + wave2) / 2;
        
        // Scale to desired range (±range)
        return combinedWave * (range / 3);
    }
    ...
    private applySectorRotation(news: NewsCard) {
        ...
        if (news.sentiment.includes('negative')) {
            ...
            unaffectedSectors.forEach(sector => {
                const assetTypes = this.getAssetTypesForSector(sector);
                assetTypes.forEach(type => {
                    // Fixed positive drift (1.5%) to remove non-deterministic randomness
                    const drift = 1.5 * 100; // 1.5% as sentiment points
    ...
    public handleBuy(socketId: string, assetId: string, amount: number) {
        ...
        // Liquidity scaling: large cumulative orders cause self-slippage
        const slippage = this.calculateSlippage(player, assetId, amount, asset.currentPrice);
    ...
    private calculateSlippage(player: any, assetId: string, amount: number, price: number): number {
        // Larger orders = more slippage (prevents big players from dominating)
        // Fixed exploit: track cumulative order value in the current round for this asset
        const roundTrades = player.transactionLog.filter((t: any) => t.round === this.gameState.currentRound && t.assetId === assetId);
        const previousVolume = roundTrades.reduce((sum: number, t: any) => sum + (t.amount * t.price), 0);
        
        const currentOrderValue = amount * price;
        const totalCumulativeValue = previousVolume + currentOrderValue;
        
        if (totalCumulativeValue > 5000) return 0.02;  // 2% slippage for cumulative volume > $5000
        if (totalCumulativeValue > 2000) return 0.01;  // 1% slippage for cumulative volume > $2000
        if (totalCumulativeValue > 1000) return 0.005; // 0.5% slippage for cumulative volume > $1000
        return 0; // No slippage for small cumulative volume
    }
